...
def getPaths():
    ...
    location_collect = rospack.get_path(
        'outdoor_waypoint_nav') + "/launch/include/collect_goals.launch"
    location_send = rospack.get_path(
        'outdoor_waypoint_nav') + "/launch/include/send_goals_continuous.launch"
    location_calibrate = rospack.get_path(
        'outdoor_waypoint_nav') + "/launch/include/heading_calibration.launch"
    location_safety_node = rospack.get_path(
        'outdoor_waypoint_nav') + "/launch/include/safety_node.launch"


def joy_CB(joy_msg):
    global start_collect_btn
    global buttons_array
    buttons_array = [joy_msg.buttons[collect_btn_num], joy_msg.buttons[send_btn_num],
                     joy_msg.buttons[calibrate_btn_num], joy_msg.buttons[abort_btn_num], joy_msg.buttons[continue_btn_num]]
...
def launch_subscribers():
    ...
    rospy.Subscriber("/waypoint_following_status",
                     Bool, waypoint_following_status_CB)
...
def check_buttons():
    ...
    if (calibrate_complete or collect_complete or send_complete):
        # Sleep for 2 seconds to allow time for other nodes to shutdown
        rospy.sleep(2)
        launch.shutdown()
        print_instructions()
